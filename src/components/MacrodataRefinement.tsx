  ...
  useEffect(() => {
    ...
    const sketch = (p: p5) => {
      ...
      class Data {
        ...
        show() {
          const digitSize = this.binIt
            ? p.lerp(this.sz, baseSize * 2.5, p.map(this.binPause, 8, 0, 0, 1))
            : this.sz;
      ...
      p.draw = () => {
        ...
        if (lumonImage) {
          ...
          g.tint(palette.FG);
          g.image(lumonImage, g.width - 100, 10, 80, 40);
        }
        if (nope && nopeImage) {
          g.imageMode(p.CENTER);
          g.image(nopeImage, g.width / 2, g.height / 2);
          if (p.millis() - nopeTime > 1000) nope = false;
        }
      ...
      p.mouseReleased = () => {
        if (!refining) return;
        refining = false;
        let refinery = numbers.filter(
          (n) => n.inside(refineTX, refineTY, refineBX, refineBY) && n.refined
        );
        let total = numbers.filter((n) =>
          n.inside(refineTX, refineTY, refineBX, refineBY)
        ).length;
        if (refinery.length > 0 && refinery.length > 0.5 * total) {
          const availableBins = refined.filter((b) => b.count < b.goal);
          if (availableBins.length > 0) {
            const targetBin = p.random(availableBins);
            refinery.forEach((n) => n.refine(targetBin));
          }
        } else if (total > 0) {
          nope = true;
          nopeTime = p.millis();
